...
<template>
  <div class="bg-white rounded py-4 md:py-6">
    <div class="mb-6">
      <h3 class="text-xl font-semibold">
        No products found
      </h3>
      <p v-if="termLabel" class="mt-1 text-slate-600">
        We could not find any products matching
        <span class="font-medium text-slate-900">{{ termLabel }}</span>.
      </p>
    </div>

    <PopularCategories class="mb-8" :number-of-recommendations="8" :use-container="false"
    ...
    <section v-if="products.length > 0">
      ...
      <div class="grid gap-3 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <ProductTile v-for="(product, index) in products" :key="product.productId ?? index" :product="product" />
      </div>
    </section>
  </div>
</template>
