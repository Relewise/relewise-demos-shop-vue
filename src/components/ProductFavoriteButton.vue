...
<template>
    <div class="favorite-button inline-flex" @click.stop>
        <relewise-product-favorite-button
            v-if="!isAnonymous"
            class="favorite-button__web-component"
            :product.prop="product"
            :user.prop="user"
        />
        <button
            v-else
            class="absolute top-2 right-2 text-slate-900 bg-white/90 p-[0.35em] rounded-full shadow-[0_1px_4px_rgba(0,0,0,0.12)] cursor-not-allowed"
            type="button"
            disabled
        >
            <svg
            ...
            <span class="sr-only">Log in to add favorite</span>
        </button>
    </div>
</template>
