    ...
    <div v-bind="$attrs">
        <ul v-if="(facet.field == 'Category' || facet.field == 'Brand') && allOptions.length > 0">
            <li v-for="(option, oIndex) in options" :key="oIndex" class="flex pb-1.5">
                ...
                <label v-if="option.value && typeof option.value === 'object' && 'id' in option.value"
                    ...
                    <input class="accent-brand-500 mr-1 h-4 w-4 cursor-pointer shrink-0" type="checkbox"
                        :value="option.value.id" :checked="option.selected"
            ...
            <!-- {{ JSON.stringify(facet.$type, null, 2) }} -->
        </ul>
        <!-- <ul v-if="(facet.field == 'Data' && facet.key == 'Brand') && allOptions.length > 0"> -->
            <ul v-if="(facet.field === 'Data' && 'key' in facet && facet.key === 'Brand') && allOptions.length > 0">
            <!-- {{ JSON.stringify(options, null, 2) }} -->
            <li v-for="(option, oIndex) in options" :key="oIndex" class="flex pb-1.5">
...
const options = computed(() => {
    ...
    //Changed to take ContentDataStringFacet into consideration - it has no Displayname
