<template>
    <div class="shadow rounded overflow-hidden hover:shadow-lg transition duration-200 cursor-pointer flex flex-col" @click="handleClick">
        <div v-if="displayAd.result?.data?.ImageUrl?.value" class="w-full flex justify-center bg-gray-50">
            <!-- <img :src="displayAd.data.ImageUrl.value"></img>  -->
             <img class="object-cover" :src="`/tile${tileId}.png`">
        </div>
...
const tileId = Math.floor(Math.random() * 4) + 1;
